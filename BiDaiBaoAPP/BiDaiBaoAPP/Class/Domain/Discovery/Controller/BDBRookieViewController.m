...
#import "BDBRookieViewController.h"
...
@interface BDBRookieViewController ()<UITableViewDelegate,UITableViewDataSource>
@property (weak, nonatomic) IBOutlet UITableView *rookieTableView;
...
@implementation BDBRookieViewController
...
- (void)viewDidLoad {
    ...
    self.rookieTableView.estimatedRowHeight = 100;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == 0) {
        ...
        introductionCell.introductionLeadingImageView.image = UIImageWithName(@"discovery_rookie_img");
        introductionCell.introductionCellTitle.text = @"新手上路";
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [self performSegueWithIdentifier:@"DetailOfRookie" sender:self];
...
- (void)refreshDatas {
    ...
    parameters[@"TypeID"] = [NSString stringWithFormat:@"%d",22];
...
- (void)loadMoreDatas {
    ...
    [manager POST:requestUrl parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        [self.rookieTableView reloadData];
...
- (void)initHeaderAndFooter {
    ...
    //初始化表头部
    thisInstance.rookieTableView.header = [BDBTableViewRefreshHeader headerWithRefreshingBlock:^{
        //刷新数据
        [thisInstance refreshDatas];
        
        //刷新完数据后，回收头部
        [thisInstance.rookieTableView.header endRefreshing];
    }];
    
    //初始化表尾部
    thisInstance.rookieTableView.footer = [BDBTableViewRefreshFooter footerWithRefreshingBlock:^{
        //加载更多数据
        [thisInstance loadMoreDatas];
        
        //刷新完数据后，回收头部
        [thisInstance.rookieTableView.footer endRefreshing];
    }];
    
}

- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([@"DetailOfRookie" isEqualToString:segue.identifier]) {
