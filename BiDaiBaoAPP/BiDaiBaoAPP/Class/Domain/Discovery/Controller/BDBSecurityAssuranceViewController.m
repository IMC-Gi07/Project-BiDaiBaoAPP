//
//  BDBSecurityAssuranceViewController.m
//  BiDaiBaoAPP
//
...
#import "BDBSecurityAssuranceViewController.h"
...
@interface BDBSecurityAssuranceViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation BDBSecurityAssuranceViewController
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == 0) {
        ...
        introductionCell.introductionLeadingImageView.image = UIImageWithName(@"discovery_securityAssurance_img");
        introductionCell.introductionCellTitle.text = @"安全保障";
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [self performSegueWithIdentifier:@"DetailOfSecurityAssurance" sender:self];
...
- (void)refreshDatas {
    ...
    parameters[@"TypeID"] = [NSString stringWithFormat:@"%d",24];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([@"DetailOfSecurityAssurance" isEqualToString:segue.identifier]) {
